...
{
    public delegate void DateChangedHandler(object sender, DariDatePickerEventArgs e);
    [ToolboxBitmap(typeof(DateTimePicker)), Designer(typeof(DariCalendarDesigner)), DefaultEvent("DateChanged"), DefaultProperty("Value")]
    public partial class DariCalendar : Control
    {
        #region Fields
        private readonly string[,] _dayEvents;
        private DariDate _value;
        #endregion

        /// <summary>
        /// Occurs when the DariSelectedDate property changes.
        /// </summary>
        public event DateChangedHandler DateChanged;
        protected virtual void OnDateChanged(DariDatePickerEventArgs e)
        {
                ...
                Value.DateChanged += (newFarsiDate, oldFarsiDate) =>
                                                                   {
                                                                       ...
                                                                       SetDatePickerText();
                                                                       OnDateChanged(new DariDatePickerEventArgs(newFarsiDate, oldFarsiDate));
                                                                       this.Invalidate(true);
                                                                   };
                this.Invalidate(true);
            }
        ...
        private void SetControlMode()
        {
            ...
            {
                lblToday.Text = string.Format("تاریخ امروز: {0}", DariDateHelper.ToDariDigit(DariDateHelper.GetLongDariDate(DateTime.Now)));
            ...
            table.Columns.AddRange(new DataGridViewColumn[]
                                        {
                                            ...
                                            new DataGridViewTextBoxColumn {HeaderText = "۱"+" شنبه ", SortMode = DataGridViewColumnSortMode.NotSortable},
                                            new DataGridViewTextBoxColumn {HeaderText = "۲"+" شنبه ", SortMode = DataGridViewColumnSortMode.NotSortable},
                                            new DataGridViewTextBoxColumn {HeaderText = "۳"+" شنبه ", SortMode = DataGridViewColumnSortMode.NotSortable},
                                            new DataGridViewTextBoxColumn {HeaderText = "۴"+" شنبه ", SortMode = DataGridViewColumnSortMode.NotSortable},
                                            new DataGridViewTextBoxColumn {HeaderText = "۵"+" شنبه ", SortMode = DataGridViewColumnSortMode.NotSortable},
        ...
        {
            table.Rows.Clear();
            numYear.Value = DariDateHelper.GetSectionOfDate(currentDate, true, SectionOfDate.Year);
            numMonth.Value = DariDateHelper.GetSectionOfDate(currentDate, true, SectionOfDate.Month);
            lblCurrent.Text = string.Format("انتخاب شده: {0}", DariDateHelper.ToDariDigit(DariDateHelper.GetLongDariDate(Value.GregorianSelectedDate)));
            lblCurrent.Tag = Value.GregorianSelectedDate;
            var firstSaturday = currentDate;
            while (firstSaturday.DayOfWeek != DayOfWeek.Saturday ||
                   DariDateHelper.GetSectionOfDate(firstSaturday, true, SectionOfDate.Month) ==
                   DariDateHelper.GetSectionOfDate(currentDate, true, SectionOfDate.Month))
                firstSaturday = firstSaturday.AddDays(-1);
            var index = 0;
            DataGridViewCell currentCell = null;
            var dayEvents = DariDateHelper.CalendarEvents.Where(c =>
                    c.Month >= DariDateHelper.GetSectionOfDate(firstSaturday, c.IsDari, SectionOfDate.Month, (c.IsDari || DariDateHelper.CalendarHijriAdjustment == null ? 0 : DariDateHelper.CalendarHijriAdjustment[c.Month])) ||
                    c.Month <= DariDateHelper.GetSectionOfDate(firstSaturday.AddDays(42), c.IsDari, SectionOfDate.Month, (c.IsDari || DariDateHelper.CalendarHijriAdjustment == null ? 0 : DariDateHelper.CalendarHijriAdjustment[c.Month]))).ToList();
            for (var i = 0; i < 6; i++)
            {
                table.Rows.Add(1);
                for (var j = 0; j < 7; j++)
                {
                    ...
                    table.Rows[i].Cells[j].Value = DariDateHelper.ToDariDigit(DariDateHelper.GetSectionOfDate(newDate, true, SectionOfDate.Day).ToString("00"));
                    table.Rows[i].Cells[j].Tag = newDate;
                    _dayEvents[i, j] = string.Empty;
                    var todayEvents = dayEvents.Where(c => c.Month == DariDateHelper.GetSectionOfDate(newDate, c.IsDari, SectionOfDate.Month, (c.IsDari || DariDateHelper.CalendarHijriAdjustment == null ? 0 : DariDateHelper.CalendarHijriAdjustment[c.Month])) &&
                                                           c.Day == DariDateHelper.GetSectionOfDate(newDate, c.IsDari, SectionOfDate.Day, (c.IsDari || DariDateHelper.CalendarHijriAdjustment == null ? 0 : DariDateHelper.CalendarHijriAdjustment[c.Month]))).ToList();

                    bool isHolidayForce = (DariDateHelper.tarikhKhasHoliday != null) ? 
                        (DariDateHelper.tarikhKhasHoliday.Contains(newDate.ToString("yyyy/MM/dd")) ? true : false) : false;
                    ...
                    if (DariDateHelper.GetSectionOfDate(currentDate, true, SectionOfDate.Year) != DariDateHelper.GetSectionOfDate(newDate, true, SectionOfDate.Year) ||
                        DariDateHelper.GetSectionOfDate(currentDate, true, SectionOfDate.Month) != DariDateHelper.GetSectionOfDate(newDate, true, SectionOfDate.Month))
        ...
        private void SetDatePickerText()
        {
            ...
            var str = Value.Format == DateFormat.Short
                                     ? DariDateHelper.ToDariDigit(DariDateHelper.GetShortDariDate(gDate))
                                     : DariDateHelper.ToDariDigit(DariDateHelper.GetLongDariDate(gDate));
        ...
        private void table_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Value.DariSelectedDate = DariDateHelper.GetShortDariDate(gDate);
            popupDatePicker.HideDropDown();
        }
        ...
        {
            var selectedYearAndMonth = DariDateHelper.GetGregorianDate(string.Format("{0}/{1}/{2}", numYear.Value, numMonth.Value, 1));
            var gDate = Value.GregorianSelectedDate;
            var isSameMonth = DariDateHelper.GetSectionOfDate(gDate, true, SectionOfDate.Year) == numYear.Value &&
                              DariDateHelper.GetSectionOfDate(gDate, true, SectionOfDate.Month) == numMonth.Value;
            CreateCells(isSameMonth ? gDate : selectedYearAndMonth);
        }

        private void lblToday_Click(object sender, EventArgs e)
        {
            Value.DariSelectedDate = DariDateHelper.GetShortDariDate((DateTime)lblToday.Tag);
            popupDatePicker.HideDropDown();
        }
        ...
        private void lblCurrent_Click(object sender, EventArgs e)
        {
            ...
            Value.DariSelectedDate = DariDateHelper.GetShortDariDate(currentDate);
        }
        #endregion
    }
